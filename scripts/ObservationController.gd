...
func enable_observation() -> void:
	darkness.color = Color(GlobalData.status.get_observation_darkness(), GlobalData.status.get_observation_darkness(), GlobalData.status.get_observation_darkness(), 1.0)
	if spotlight:
		spotlight.visible = true
		spotlight.enabled = true
		spotlight.texture_scale = GlobalData.status.get_observation_vision_radius() / 100.0
...
func update_camera(delta: float) -> void:
	...
	if move_dir.length() > 0:
		camera.global_position += move_dir.normalized() * GlobalData.status.get_observation_camera_speed() * delta
...
func update_visible_holds() -> void:
	...
	for hold in get_tree().get_nodes_in_group("hold"):
		...
		hold.is_currently_visible = (distance <= GlobalData.status.get_observation_vision_radius())
