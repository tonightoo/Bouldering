...
func execute(key: String, player: Player, stage: Stage):
	...
	executed_tween.tween_method(
		func(value: float): 
			GlobalData.status.left_arm_length_multiplier = value
			player.resize_left_arm(executed_tween),
		1.0,   # 開始値
		5.0,   # 終了値
		0.5    # 何秒かけて伸ばすか
	)
...
func shrink(player: Player) -> void:
	...
	tween.tween_method(
		...
		GlobalData.status.left_arm_length_multiplier,   # 開始値
		1.0,   # 終了値
		0.5    # 何秒かけて伸ばすか
	)
